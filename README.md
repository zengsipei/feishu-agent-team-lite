...
- [1Panel Read-Only Pre-Check](docs/1panel-readonly-precheck.md)：正式部署前的服务器只读检查、Bash/PowerShell 脱敏证据包模板、Manual 1Panel Deploy Gate 和禁止操作。
- [1Panel Read-Only Evidence](docs/1panel-readonly-evidence.md)：当前阶段的脱敏证据包、本地只读验证结果、目标服务器待确认项和手动部署门禁状态。
- [Manual 1Panel Deploy Gate](docs/1panel-manual-deploy-gate.md)：人工 1Panel 部署窗口的确认项、允许动作、部署后严格证据和回传模板。
...
当前下一阶段是 Manual 1Panel Deploy Gate：它只授权人工在 1Panel 中执行部署窗口，不授权 Agent 直接操作服务器，也不是接入远程 CI/CD 部署。
